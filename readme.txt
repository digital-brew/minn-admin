...
* **Content** — posts, pages and custom post types sorted by publish date (scheduled posts lead), with search, category/tag filters, status pills (live posts carrying unsaved edits wear a Modified chip, with a matching filter; a post someone else has open says who is editing), bulk actions, and right-click row actions for quick publish/draft/trash, view, and a block-editor escape.
...
* **Comments** — moderation with Pending/Approved/Spam/Trash tabs, one-click actions, bulk moderation with shift-range select, inline replies, in-place comment editing, Block commenter (adds them to core's disallowed list, with Undo), and a right-click menu.
* **Ecommerce** — WooCommerce day-to-day in Minn: Orders open as full pages (linkable, refreshable, with a Quick view modal from the list) with status, notes, a Payment card that records hand-received payments through WooCommerce's own machinery, itemized refunds with quantity steppers and restocking, resend/custom email, pay URL, New order and Analytics; the Overview carries a Store strip naming the orders that need work today. Products (stock filters, bulk, daily fields, Add product), Coupons, Customers, and Subscriptions when WooCommerce Subscriptions is active. Orders, products and customers all carry right-click menus for the common moves (status changes, stock and publish toggles, email, jump to a customer's orders). Invoice/packing-slip downloads when PDF Invoices & Packing Slips is active.
...
= 0.20.0 =
* Consequential actions move to a shared confirm dialog with scope disclosure: updates enumerate what changes and what stays untouched, permanent deletes wear danger styling with plain-stakes copy, and quick reversible verbs keep one-click confirms.
* Comments gain in-place editing and a Block commenter action (core's disallowed list, with Undo); the content list shows who is editing a post right now via core's own edit lock.
* Settings closes two more Customizer-era gaps (site logo, site language); Search & Filter Pro and Admin Columns Pro join the license manager.
* Plugin activation runs through admin-ajax, a real admin context, so activation hooks that assume wp-admin no longer fail; add-on families (like Admin Columns Pro's) keep their distinguishing names in Extensions.

= 0.19.0 =
* Orders open as full pages at /minn-admin/orders/{id} with the modal surviving as Quick view; a Payment card records hand-received payments; refunds gain per-item quantity steppers, restocking, gateway-aware copy and a delete-refund undo path; the Overview gains a Store strip naming the orders that need work today.
* The System log viewer grows sources: the WordPress debug log, the PHP error log and every WooCommerce log channel, with a collapse-repeats mode and a minn_admin_log_sources filter for plugins and hosts.
* Live posts carrying unsaved edits wear a Modified chip and filter in the content list, backed by a minn_modified REST field on every REST-visible post type.
* A server-side lock guard rejects saves racing a takeover; Yes/No notices extract and answer properly (button elements join the CTA walk); reloads after plugin-flipping actions retry a dropped socket once. Full details in changelog.md.
