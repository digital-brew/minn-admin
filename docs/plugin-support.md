...
| **Activity log** | Simple History, WP Activity Log, Aryo, Stream, **Wordfence**, **Limit Login Attempts Reloaded**, **Solid Security** | **Activity Log** surface (Wordfence = login security; Limit Login Attempts and Solid Security = lockout logs with status cards and one-click Unlock/Release through each plugin's own store) |
...
| **Analytics** | Koko, WP Statistics, Burst, Independent Analytics, AnalyticsWP, **Site Kit** | Overview **Traffic** chart (daily visitors/pageviews). **Koko** also powers the day-click drill-down: top pages + referrers for the bar's window via `minn_admin_traffic_day` |
