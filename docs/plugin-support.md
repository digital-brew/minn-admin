...
| **Media** | Regenerate Thumbnails, Safe SVG | ↻ Thumbnails button on the media detail modal (per-image full rebuild). Safe SVG: **SVG** filter tab when uploads are allowed; detail note; sanitization stays Safe SVG's |
