...
| **Ecommerce** | WooCommerce | **Orders** surface (list, search, status, refunds, pay URL, resend/custom email) + **Products** surface (list, search by name/SKU/ID, status tabs, daily fields: name, SKU, price, stock, visibility, short description; variable/grouped price/stock deep-link to WooCommerce) + Overview stats. The product CPT is fenced out of Content so the writing editor never opens a catalog item |
