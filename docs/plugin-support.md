...
| **Backups** | UpdraftPlus, Disembark, Duplicator, WPvivid | **Backups** surface; health check + "Back up now" (UpdraftPlus, else WPvivid); status card, CLI command, sessions + cleanup (Disembark); package list with disk sizes, status card and delete-through-its-own-cleanup (Duplicator, no freshness claims: manual builds); backup list + status card + schedule + backup-now + delete-through-its-own-cleanup (WPvivid) |
...
2. **Backups providers** — WPvivid shipped (list/status/schedule/backup-now/
   delete; free tier schedules so freshness is claimable). Still open:
   BackWPup, and an All-in-One WP Migration local-exports listing (never
   claim freshness; the Disembark precedent).
